/** Mirror HTML — Shopify analytics / tracking (404 ve konsol gürültüsü) */

const SHOPIFY_SCRIPT_SRC =
  /<script\b[^>]*\ssrc=["'][^"']*(?:shopifycloud|trekkie\.storefront|shop_events_listener|privacy-banner|portable-wallets|shopify-perf-kit|origin_trials|monorail-edge)[^"']*["'][^>]*>\s*<\/script>/gi;

const SHOPIFY_SCRIPT_IDS =
  /<script\b[^>]*\bid=["'](?:web-pixels-manager-setup|shop-js-analytics|scb4127)["'][^>]*>[\s\S]*?<\/script>/gi;
...
const DNS_PREFETCH_MONORAIL = /<link\b[^>]*\bhref=["']https:\/\/monorail-edge\.shopifysvc\.com[^"']*["'][^>]*>/gi;

const STUB = `<script id="kn-shopify-stub">(function(){window.Shopify=window.Shopify||{};var lf=function(){var q=[];function fn(){q.push([].slice.call(arguments));var cb=arguments[0]&&arguments[0][0]&&arguments[0][0].onLoad;if(typeof cb==="function")try{cb()}catch(e){}}fn.q=q;return fn;};window.Shopify.loadFeatures=window.Shopify.loadFeatures||lf();window.Shopify.autoloadFeatures=window.Shopify.autoloadFeatures||lf();window.ShopifyAnalytics=window.ShopifyAnalytics||{meta:{},lib:{}};var t=window.ShopifyAnalytics.lib;["page","track","identify","ready","trackForm","trackLink"].forEach(function(m){t[m]=t[m]||function(){}});window.trekkie=window.trekkie||[];window.trekkie.load=function(){};window.trekkie.ready=function(fn){if(typeof fn==="function")try{fn()}catch(e){}};window.webPixelsManager=window.webPixelsManager||{init:function(){return{publishCustomEvent:function(){},visitor:function(){return{}}}}};})();</script>`;

/** HTTrack mirror — gereksiz Shopify JS isteklerini kaldırır */
export function stripShopifyTrackingFromMirrorHtml(html: string): string {
  ...
  out = out.replace(DNS_PREFETCH_MONORAIL, "");

  if (!out.includes('id="kn-shopify-stub"')) {
    out = out.replace(/<head>/i, `<head>${STUB}`);
  }

  return out;
}

/** Shopify / myshopify bağlantılarını yerel mağaza rotalarına çevir */
export function rewriteShopifyLinksInMirrorHtml(html: string): string {
  let out = html;

  out = out.replace(
    /https?:\/\/shopify\.com\/\d+\/account[^"'\s]*/gi,
    "/account",
  );
  out = out.replace(
  ...
  out = out.replace(
    /https?:\/\/(?:www\.)?theking-noor\.myshopify\.com\/products\/([^"?\s#]+)/gi,
    "/products/$1",
  );
  out = out.replace(
    /https?:\/\/(?:www\.)?theking-noor\.myshopify\.com\/collections\/([^"?\s#]+)/gi,
    "/collections/$1",
  );
  out = out.replace(
  ...
  out = out.replace(
    /cart_add_url:\s*['"]\/cart\/add['"]/gi,
    "cart_add_url: '/api/cart/items'",
  );
  out = out.replace(
    /cart_change_url:\s*['"]\/cart\/change['"]/gi,
    "cart_change_url: '/api/cart/items'",
  );
  out = out.replace(
    /cart_update_url:\s*['"]\/cart\/update['"]/gi,
    "cart_update_url: '/api/cart'",
  );
  out = out.replace(/cart_url:\s*['"]\/cart['"]/gi, "cart_url: '/cart'");
  out = out.replace(
    /(?:\.\.\/)*(?:[a-z]{2}(?:-[a-z]{2})?\/)?search\.html/gi,
    "/search",
  );
