...
function injectFaviconLinks(html: string, favicon: string): string {
  const versioned = withBrandAssetVersion(favicon);
  const u = escAttr(versioned);
...
function buildHeadBootstrap(branding: MirrorBranding): string {
  ...
  return `<script id="kn-branding-bootstrap">(function(){var P=${payload};function bust(u){if(!u)return u;var p=(u.split("?")[0]||"").replace(/[^\\w-]/g,"").slice(-32)||"1";var s=u.replace(/([?&])(v|kn)=[^&]*/g,"").replace(/[?&]$/,"");return s+(s.indexOf("?")>=0?"&":"?")+"v="+p;}function pathOf(u){return(u||"").split("?")[0];}function unified(){return!!document.querySelector("[id^=kn-logo-unify-script]");}function set(el,u,fallback){if(!el||!u)return;var n=bust(u);var fb=fallback?bust(fallback):"";el.removeAttribute("data-src");el.removeAttribute("width");el.removeAttribute("height");el.onerror=function(){if(fb&&pathOf(el.src)!==pathOf(fb)){el.onerror=null;set(el,fb,"");return;}el.style.visibility="hidden";};el.src=n;el.setAttribute("srcset",n+" 1x, "+n+" 2x");}function apply(){if(unified()){if(P.light)document.querySelectorAll("img.footer--logo-img").forEach(function(el){set(el,P.light,P.fallbackLight);});}else{if(P.logo)document.querySelectorAll("img.header--logo-img:not(.transparent-logo-img)").forEach(function(el){set(el,P.logo,P.fallbackDark);});if(P.light)document.querySelectorAll("img.transparent-logo-img,img.footer--logo-img").forEach(function(el){set(el,P.light,P.fallbackLight);});}if(!unified())document.querySelectorAll('link[rel="preload"][as="image"]').forEach(function(l){if(/logo/i.test(l.href))l.href=bust(P.light||P.logo);});}function boot(){apply();var moScheduled=false;new MutationObserver(function(){if(unified())return;if(moScheduled)return;moScheduled=true;requestAnimationFrame(function(){moScheduled=false;apply();});}).observe(document.documentElement,{subtree:true,attributes:true,attributeFilter:["src","srcset"]});}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",boot);else boot();})();</script>`;
}
