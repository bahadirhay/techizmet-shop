...
export default async function AccountRegisterPage({
  ...
  const useThemeShell =
    homepageMode === "mirror" &&
    isThemeShellEnabledForCommercePath("/account/register", query, themeShellLive);

  if (useThemeShell) {
    const locale = await getStoreLocale();
    const content = await resolveThemeShellAccountAuthContent(site.id, locale, "register");
    if (!content) notFound();
    return (
