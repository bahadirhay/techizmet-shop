...
function buildShareScript(share: ProductSharePayload): string {
  ...
  function closeModal(){
    if(backdrop){backdrop.classList.remove('kn-share-open');document.body.style.overflow='';}
  }
  function openModal(){
    if(!root){
      ...
      backdrop=root.querySelector('.kn-share-backdrop');
    ...
    backdrop.classList.add('kn-share-open');
    document.body.style.overflow='hidden';
    if(navigator.share){
      var native=document.getElementById('kn-share-native');
      if(!native){
        native=document.createElement('button');
        native.id='kn-share-native';
        native.type='button';
        ...
        root.querySelector('.kn-share-dialog').insertBefore(native,root.querySelector('.kn-share-channels'));
      }
    }
  }
  function injectShareButton(){
    ...
    btn.addEventListener('click',function(e){
      ...
      if(navigator.share&&!window.matchMedia('(min-width:768px)').matches){
        navigator.share({title:SHARE.title,text:SHARE.priceLabel,url:productUrl()}).catch(openModal);
      }else{openModal();}
    });
    var anchor=qs('#MainContent .product--form-actions')||
      qs('#MainContent .product--add-to-cart-wrapper')||
      qs('#MainContent [data-add-to-cart]')&&qs('#MainContent [data-add-to-cart]').parentElement;
    if(anchor){anchor.appendChild(btn);return;}
    var sticky=qs('.sticky-buy-button-wrapper, sticky-buy-button');
    if(sticky){sticky.appendChild(btn);}
  }
  if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',injectShareButton);}
  else{injectShareButton();}
})();`;
}

const SHARE_STYLES = `<style id="kn-product-share-styles">
.kn-share-trigger{margin-left:.75rem;padding:.5rem 1rem;border:1px solid currentColor;border-radius:9999px;background:transparent;font-size:.875rem;cursor:pointer;opacity:.9}
.kn-share-trigger:hover{opacity:1}
#kn-share-root{position:fixed;inset:0;z-index:10050;pointer-events:none}
.kn-share-backdrop{position:absolute;inset:0;background:rgba(0,0,0,.45);opacity:0;transition:opacity .2s;pointer-events:none}
.kn-share-dialog{position:absolute;left:50%;bottom:0;transform:translate(-50%,100%);width:min(100%,28rem);max-height:85vh;overflow:auto;background:#fff;border-radius:1rem 1rem 0 0;padding:1.25rem 1rem 1.5rem;box-shadow:0 -8px 32px rgba(0,0,0,.15);transition:transform .25s ease;pointer-events:auto}
.kn-share-open{pointer-events:auto}
.kn-share-open .kn-share-backdrop{opacity:1;pointer-events:auto}
.kn-share-open .kn-share-dialog{transform:translate(-50%,0)}
@media(min-width:768px){
  .kn-share-dialog{bottom:auto;top:50%;transform:translate(-50%,-40%);border-radius:1rem;opacity:0}
  .kn-share-open .kn-share-dialog{transform:translate(-50%,-50%);opacity:1}
}
...
.kn-share-img{width:4rem;height:4rem;object-fit:cover;border-radius:.5rem;background:#f4f4f5}
...
#kn-share-toast{position:fixed;bottom:1.5rem;left:50%;transform:translateX(-50%) translateY(120%);background:#18181b;color:#fff;padding:.5rem 1rem;border-radius:.5rem;font-size:.8125rem;z-index:10060;transition:transform .2s}
...
export function injectMirrorProductShareHtml(html: string, share: ProductSharePayload): string {
  ...
  if (!out.includes('id="kn-product-share-styles"')) {
    out = out.replace(/<\/head>/i, `${styles}</head>`);
  }
