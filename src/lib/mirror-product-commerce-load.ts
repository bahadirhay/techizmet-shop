...
function productHasDisplayPrice(
  product: { priceMinor: number; compareAtMinor: number | null },
  variants: VariantRow[],
  memberPricing: CustomerGroupPricing | null,
): boolean {
  if (variants.length) {
    return variants.some((v) => variantCatalogPrices(v, memberPricing).unitMinor > 0);
  }
  return (
    variantCatalogPrices(
...
export async function loadMirrorProductCommerceUncached(
  ...
  const variantPayload = variants.map((v) => {
    ...
    return {
      ...
      priceLabel: p.unitMinor > 0 ? formatTry(p.unitMinor) : "",
  ...
  const showPrice = productHasDisplayPrice(product, variants, memberPricing);
  let priceLabel = "";
  ...
  const sharePriceLabel = showPrice
    ? fromPrice
      ? `${texts?.startingPricePrefix ?? "Başlayan fiyat"}: ${priceLabel}`
      : priceLabel
    : "";
